...
// Gallery photo data now lives in /public/data/gallery-photos.json.
// Use getPhotos() to fetch it client-side instead of importing a static array.
const GITHUB_URL =
  "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/gallery/gallery-photos.json";

const LOCAL_URL = "/data/gallery/gallery-photos.json";

export async function getPhotos(): Promise<ExtendedPhoto[]> {
  try {
    ...
    console.warn("Using local gallery data:", error);

    const localRes = await fetch(LOCAL_URL);

    if (!localRes.ok) {
      throw new Error("Failed to load gallery photos from both GitHub and local.");
    }
