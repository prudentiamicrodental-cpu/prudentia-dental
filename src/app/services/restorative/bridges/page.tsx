...
export default function Bridges() {
  ...
  const [content, setContent] = useState<BridgesData | null>(null);
  const [loadError, setLoadError] = useState(false);
  const [openFaq, setOpenFaq] = useState<number | null>(0);

  useEffect(() => {
    function isValidSchema(data: any): data is BridgesData {
      return (
        !!data &&
        typeof data === "object" &&
        !!data.hero &&
    ...
    async function loadData() {
      const GITHUB_URL =
        "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/service/restorative/bridges.json";

      const LOCAL_URL = "/data/service/restorative/bridges.json";
      ...
      try {
        ...
        const data: BridgesData = await res.json();
        ...
        try {
          ...
          const localData: BridgesData = await localRes.json();
          ...
          console.error("Failed to load valid bridges data:", localError);
          setLoadError(true);
        }
      }
    }
    ...
    <div className="min-h-screen overflow-hidden bg-gradient-to-br from-purple-50 via-white to-purple-50">
            ...
            <div className="mt-12">
              <div className="bg-white bg-opacity-10 rounded-3xl p-8 backdrop-blur-sm border border-white/20 max-w-4xl mx-auto relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg mb-8">
                <Image
                  ...
                  alt="Dental bridges near me in Pimple Saudagar"
                  fill
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <section className="mb-28">
          ...
          >
            <h2 className="text-3xl md:text-4xl font-bold text-purple-800 mb-8 text-center">
              <Markdown inline>{content.whatAreBridges.title}</Markdown>
            </h2>
            <p className="text-lg text-purple-700 leading-relaxed mb-10 text-center max-w-3xl mx-auto">
              <Markdown inline>{content.whatAreBridges.description}</Markdown>
            </p>
            <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-4 mb-10">
              {content.whatAreBridges.items.map((item, index) => (
                <div
                  key={index}
            ...
            <p className="text-purple-600 italic text-center leading-relaxed">
              <Markdown inline>{content.whatAreBridges.footnote}</Markdown>
            </p>
          </div>
        </section>
        ...
        <section className="mb-28">
          <div className="text-center mb-12" style={staggerDelay(11)}>
            <h2 className="text-3xl md:text-4xl font-bold text-purple-800 mb-12">
              <Markdown inline>{content.bridgeTypes.title}</Markdown>
            </h2>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {content.bridgeTypes.items.map((item, index) => (
              <div
                key={index}
        ...
        <section className="mb-28">
          <div className="text-center mb-16" style={staggerDelay(16)}>
            <h2 className="text-4xl md:text-6xl font-bold text-purple-800 mb-12">
              <Markdown inline>{content.bridgesTechnology.title}</Markdown>
            </h2>
            ...
            <div className="bg-white rounded-3xl p-8 shadow-xl inline-block mb-12 border border-purple-100 relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg mb-8">
              <Image
                urlEndpoint={process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}
                src={content.bridgesTechnology.image}
                alt="Dental bridge technology"
                fill
                className="object-contain"
              />
            </div>
            ...
            <p className="text-xl text-purple-700 max-w-5xl mx-auto leading-relaxed mb-16">
              <Markdown inline>{content.bridgesTechnology.description}</Markdown>
            </p>
          </div>

          <div className="grid md:grid-cols-3 gap-8 mb-16">
            {content.bridgesTechnology.features.map((feature, index) => {
          ...
          >
            <div className="bg-purple-500 bg-opacity-20 rounded-2xl p-8 border border-purple-400/30">
              <p className="text-lg text-center leading-relaxed">
                <Markdown inline>{content.bridgesTechnology.highlight}</Markdown>
              </p>
            </div>
          </div>
        </section>
