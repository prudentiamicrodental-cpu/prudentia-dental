...
export default function JawPainTreatmentPage() {
  const { handleOpenChatbot } = useChatbot();
  const [isVisible, setIsVisible] = useState(false);
  const [content, setContent] = useState<JawPainData | null>(null);
  const [openFaq, setOpenFaq] = useState<number | null>(0);

  useEffect(() => {
    async function loadData() {
      const GITHUB_URL =
        "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/service/restorative/jaw-pain.json";

      const LOCAL_URL = "/data/service/restorative/jaw-pain.json";

      try {
        const res = await fetch(GITHUB_URL);
        if (!res.ok) throw new Error("GitHub fetch failed");
        const data: JawPainData = await res.json();
        setContent(data);
      } catch (error) {
        ...
        try {
          ...
          const localData: JawPainData = await localRes.json();
          ...
          >
            ...
            <div className="bg-white bg-opacity-15 rounded-3xl p-10 backdrop-blur-sm border border-white/20 mb-12 relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
              <Image
                ...
                alt="Jaw pain treatment results"
                fill
                className="object-contain"
                priority
              />
