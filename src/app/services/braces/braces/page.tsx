...
interface BracesTreatmentFullData {
  meta: MetaData;
  hero: {
    title: string;
    highlightedText: string;
...
export default function BracesTreatmentFullPage() {
  const { handleOpenChatbot } = useChatbot();
  const [isVisible, setIsVisible] = useState(false);
  const [content, setContent] = useState<BracesTreatmentFullData | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const data: BracesTreatmentFullData = await res.json();
        setContent(data);
      } catch (error) {
        ...
        try {
          ...
          const localData: BracesTreatmentFullData = await localRes.json();
