...
interface ClearAlignersFullData {
  meta: MetaData;
  hero: {
    title: string;
    highlightedText: string;
...
export default function ClearAlignersFullPage() {
  const { handleOpenChatbot } = useChatbot();
  const [isVisible, setIsVisible] = useState(false);
  const [content, setContent] = useState<ClearAlignersFullData | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        ...
        const data: ClearAlignersFullData = await res.json();
        setContent(data);
      } catch (error) {
        ...
        try {
          ...
          const localData: ClearAlignersFullData = await localRes.json();
