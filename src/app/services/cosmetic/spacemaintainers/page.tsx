...
interface SpaceMaintainersData {
  meta: MetaData;
  hero: HeroData;
...
const EMPTY_DATA: SpaceMaintainersData = {
...
const SpaceMaintainersPage = () => {
    const [data, setData] = useState<SpaceMaintainersData>(EMPTY_DATA);
    const { handleOpenChatbot } = useChatbot();
  useEffect(() => {
    async function loadData() {
      const GITHUB_URL =
        "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/service/cosmetic/spacemaintainers.json";

      const LOCAL_URL = "/data/service/cosmetic/spacemaintainers.json";
      ...
      try {
        ...
        const data: SpaceMaintainersData = await res.json();
        setData(data);
      } catch (error) {
        ...
        try {
          ...
          const localData: SpaceMaintainersData = await localRes.json();
          setData(localData);
        } catch (localError) {
                ...
                <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-xl shadow-lg mb-4 overflow-hidden">
                  {hero.image && (
                    <Image
                      ...
                      src={hero.image}
                      alt="Space maintainers at Prudentia Micro Dental Care"
                      fill
                  ...
                  {whatIsSection.images.map((image, index) => (
                    <div key={index} className="relative h-48 sm:h-56 md:h-64 w-full rounded-xl overflow-hidden shadow-lg">
                      <Image
                        urlEndpoint={process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}
                        src={image}
                        alt="Dental examination for pediatric space maintainer planning"
                        fill
                ...
                <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
                  {whyChoose.image && (
                    <Image
                      urlEndpoint={process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}
                      src={whyChoose.image}
                      alt="Why choose Prudentia for space maintainers"
                      fill
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 50vw"
                      className="object-cover"
                    />
                ...
                <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
                  {processing.image && (
                    <Image
                      ...
                      alt="Space maintainer procedure"
                      fill
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 50vw"
                      className="object-cover"
                    />
...
export default SpaceMaintainersPage;