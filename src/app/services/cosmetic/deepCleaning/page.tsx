...
export default function DeepCleaningTreatmentPage() {
  const { handleOpenChatbot } = useChatbot();
  const [isVisible, setIsVisible] = useState(false);
  const [content, setContent] = useState<DeepCleaningData | null>(null);
  const [openFaq, setOpenFaq] = useState<number | null>(0);

  useEffect(() => {
    async function loadData() {
      const GITHUB_URL =
        "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/service/cosmetic/deepcleaning.json";

      const LOCAL_URL = "/data/service/cosmetic/deepcleaning.json";

      try {
        const res = await fetch(GITHUB_URL);
        if (!res.ok) throw new Error("GitHub fetch failed");
        const data: DeepCleaningData = await res.json();
        setContent(data);
      } catch (error) {
        ...
        try {
          ...
          const localData: DeepCleaningData = await localRes.json();
            ...
            <div className="mt-16">
              <div className="bg-white bg-opacity-15 rounded-3xl p-10 backdrop-blur-sm border border-white/20 max-w-5xl mx-auto relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg mb-8">
                <Image
                  ...
                  alt="Deep cleaning scaling and root planing"
                  fill
          ...
          <div className="text-center mb-20" style={staggerDelay(9)}>
            ...
            <div className="bg-white rounded-3xl p-10 shadow-2xl inline-block mb-16 border border-gray-100 relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
              <Image
                ...
                alt="What is deep cleaning scaling and root planing"
                fill
                className="object-contain"
                priority
              />
          ...
          >
            ...
            <div className="bg-white bg-opacity-15 rounded-3xl p-10 backdrop-blur-sm border border-white/20 mb-12 relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
              <Image
                ...
                alt="Deep cleaning treatment results"
                fill
                className="object-contain"
                priority
              />
