...
const EMPTY_DATA: BrokenToothData = {
...
const BrokenToothTreatmentPage = () => {
  const [data, setData] = useState<BrokenToothData>(EMPTY_DATA);
  ...
  useEffect(() => {
    async function loadData() {
      const GITHUB_URL =
        "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/service/cosmetic/brokenteeth.json";

      const LOCAL_URL = "/data/service/cosmetic/brokenteeth.json";
      ...
      try {
        ...
        const data: BrokenToothData = await res.json();
        setData(data);
      } catch (error) {
        ...
        try {
          ...
          const localData: BrokenToothData = await localRes.json();
          setData(localData);
        } catch (localError) {
  ...
  const openChatbot = handleOpenChatbot || (() => {});
  ...
  return (
    <>
      ...
      <div className="min-h-screen overflow-hidden bg-gradient-to-b from-purple-50 to-white">
                ...
                <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-xl shadow-lg mb-4 overflow-hidden">
                  {hero.image && (
                    <Image
                      ...
                      src={hero.image}
                      alt="Broken tooth treatment at Prudentia Micro Dental Care"
                      fill
        ...
        {/* What Is Broken Tooth Treatment Section */}
                  ...
                  {whatIsSection.images.map((image, index) => (
                    <div key={index} className="relative h-48 sm:h-56 md:h-64 w-full rounded-xl overflow-hidden shadow-lg">
                      <Image
                        urlEndpoint={process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}
                        src={image}
                        alt="Dental examination for broken tooth diagnosis"
                        fill
                ...
                <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
                  {whyChoose.image && (
                    <Image
                      urlEndpoint={process.env.NEXT_PUBLIC_IMAGEKIT_URL_ENDPOINT}
                      src={whyChoose.image}
                      alt="Why choose Prudentia for broken tooth treatment"
                      fill
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 50vw"
                      className="object-cover"
                    />
                ...
                <div className="relative h-64 sm:h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg">
                  {processing.image && (
                    <Image
                      ...
                      alt="Broken tooth treatment procedure"
                      fill
                      sizes="(max-width: 768px) 100vw, (max-width: 1024px) 50vw, 50vw"
                      className="object-cover"
                    />
                ...
                {faq.items.map((item, index) => (
                  ...
                  >
                    <button
                      ...
                      className="w-full text-left p-4 sm:p-6 flex justify-between items-center hover:bg-gray-100 transition-colors"
                    >
...
export default BrokenToothTreatmentPage;