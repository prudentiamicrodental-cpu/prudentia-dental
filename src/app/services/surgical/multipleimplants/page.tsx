...
interface TypesOfImplantsItem {
  title: string;
  description: string;
}

interface TypesOfImplantsData {
  title: string;
  items: TypesOfImplantsItem[];
}
...
interface ImplantsData {
  ...
  whatIs: WhatIsData;
  typesOfImplants?: TypesOfImplantsData;
...
const EMPTY_DATA: ImplantsData = {
  ...
  typesOfImplants: { title: '', items: [] },
  whoCanBenefit: { title: '', intro: '', image: '', items: [], footer: '' },
  procedure: { title: '', steps: [], footer: '' },
  benefits: { title: '', image: '', items: [], footer: '' },
...
useEffect(() => {
    ...
    async function loadData() {
      const GITHUB_URL =
        "https://raw.githubusercontent.com/prudentiamicrodental-cpu/Content/main/service/surgical/multipleimplants.json";
      const LOCAL_URL = "/data/service/surgical/multipleimplants.json";
      ...
      try {
        ...
        console.warn("Failed to load data from GitHub, falling back to local:", error);
      }
      ...
      try {
        ...
        console.warn("Failed to load local data, using empty data:", error);
  ...
  // Safe access helper
  const safeData = (field: keyof ImplantsData) => {
    return data[field] || {};
  };
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-purple-50 to-indigo-50">
      <Head>
        <title>{data.meta?.title || 'Multiple Teeth Implants'}</title>
              ...
              <div className="bg-white bg-opacity-10 p-6 rounded-2xl backdrop-blur-sm relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg mb-8">
                {data.hero?.image && (
                  <Image
                    ...
                    src={data.hero.image}
                    alt="Multiple teeth dental implants"
                    fill
      ...
      <div className="py-16 lg:py-24 bg-gradient-to-r from-gray-50 to-slate-50">
        <div className="container mx-auto px-4">
          <div
            className="max-w-4xl mx-auto text-center"
            id="why-choose-us"
      ...
      {/* What Is Section */}
      <div className="py-16 lg:py-24 bg-gradient-to-r from-purple-50 to-indigo-50">
        <div className="container mx-auto px-4">
          <div
      ...
      {data.typesOfImplants && data.typesOfImplants.items && data.typesOfImplants.items.length > 0 && (
        <section className="py-16 lg:py-24">
          <div className="container mx-auto px-4">
            <div className="max-w-6xl mx-auto">
              <div
                className="text-center mb-16"
                id="types-of-implants-title"
                data-animate
                style={fadeInUp('types-of-implants-title')}
              >
                <h2 className="text-3xl lg:text-5xl font-bold text-gray-800 mb-8">
                  <Markdown inline>{data.typesOfImplants.title}</Markdown>
                </h2>
              </div>

              <div className="grid md:grid-cols-2 gap-8">
                {data.typesOfImplants.items.map((item, index) => (
                  <div
                    key={index}
                    className="bg-white rounded-2xl shadow-lg border-2 border-purple-100 p-8 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1"
                    id={`types-of-implants-${index}`}
                    data-animate
                    style={index % 2 === 0 ? slideInLeft(`types-of-implants-${index}`) : slideInRight(`types-of-implants-${index}`)}
                  >
                    <div className="flex items-start space-x-4">
                      <div className="bg-purple-50 p-3 rounded-xl shadow-md flex-shrink-0">
                  ...
                  <div className="bg-white rounded-2xl shadow-xl overflow-hidden relative h-80 md:h-96 w-full">
                    {data.whoCanBenefit.image && (
                      <Image
                        ...
                        alt="Candidate for multiple teeth implants"
                        fill
                        className="object-contain"
                        priority
                      />
      ...
      <div className="py-16 lg:py-24 bg-gradient-to-r from-gray-50 to-slate-50">
        <div className="container mx-auto px-4">
          <div className="max-w-6xl mx-auto">
            ...
            {data.procedure?.footer && (
              <p className="text-lg text-gray-700 leading-relaxed italic text-center mt-8">
                <Markdown inline>{data.procedure.footer}</Markdown>
              </p>
            )}
          </div>
        </div>
              ...
              {data.benefits?.image && (
                <div className="max-w-3xl mx-auto relative h-80 md:h-96 w-full rounded-xl overflow-hidden shadow-lg mb-8">
                  <Image
                    ...
                    alt="Benefits of multiple teeth implants"
                    fill
                    className="object-contain"
                    priority
                  />
      ...
      <section className="py-20 bg-white">
        <div className="container mx-auto px-6 text-center">
          ...
          >
            ...
            >
              {data.finalCta?.buttonText || 'Book Consultation'} <FiArrowRight className="ml-2" />
            </motion.button>
